...
@section('breadcrumb-item-active', 'Edit Event')
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('admin.events.update', $event) }}" method="POST" class="needs-validation" novalidate>
                @csrf
                @method('PUT')
                ...
                <div id="event-form-card" class="card animate__animated animate__rollIn">
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    <option value="0" @selected(old('status', $event->status) == 0)>⏳ Pending</option>
                                    <option value="1" @selected(old('status', $event->status) == 1)>✔ Confirmed</option>
                                    <option value="2" @selected(old('status', $event->status) == 2)>✖ Cancelled</option>
                                </select>
                                @error('status')
                                    <div class="text-danger mt-1">{{ $message }}</div>
                                @enderror
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <select name="association_id"
                                        ...
                                        @foreach($associations as $id => $name)
                                            <option value="{{ $id }}" @selected(old('association_id', $event->association_id) == $id)>
                                                {{ $name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            @else
                                <input type="hidden" name="association_id" value="{{ $event->association_id }}">
                            @endif
                    ...
                    <div class="card-footer text-end">
                        ...
                        @if (auth()->user()->hasAnyRole(['admin', 'superadmin', 'board']) && (auth()->user()->hasRole('superadmin') || (int) $event->association_id === (int) auth()->user()->association_id))
