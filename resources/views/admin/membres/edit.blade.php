...
<div class="row">
    <div class="col-md-12">
        ...
        <form action="{{ route('admin.membres.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
            @csrf
            @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <select name="assign_role" class="form-select @error('assign_role') is-invalid @enderror" required>
                                ...
                                @if(Auth::user()->hasRole('super_admin'))
                                    <option value="super_admin" @selected(old('assign_role', $user->getRoleNames()->first()) == 'super_admin')>Super Admin</option>
                                @endif
                                <option value="admin" @selected(old('assign_role', $user->getRoleNames()->first()) == 'admin')>Admin</option>
                                <option value="membre" @selected(old('assign_role', $user->getRoleNames()->first()) == 'membre')>Membre</option>
                            </select>
                            @error('assign_role')
                            ...
                            @if(Auth::user()->hasRole('super_admin'))
                                <select name="association_id" class="form-select @error('association_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($associations as $id => $name)
                                        <option value="{{ $id }}" @selected(old('association_id', $user->association_id) == $id)>{{ $name }}</option>
                                    @endforeach
                                </select>
                            @else
                                <input type="hidden" name="association_id" value="{{ Auth::user()->association_id }}">
                                <input type="text" class="form-control" value="{{ $associations[Auth::user()->association_id] }}" readonly>
                            @endif
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            @if($user->getFirstMedia('profile_photo'))
                                ...
                                <small class="text-muted d-block mt-2">
                                    Current:
                                    <a href="{{ route('media.custom', ['id' => $media->id, 'filename' => $media->file_name]) }}" target="_blank">
                                        View Photo ({{ $short }})
                                    </a>
                                    <br>
                        ...
                        <div class="mb-3 col-md-3">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_active" value="1" id="activeCheck" @checked(old('is_active', $user->is_active))>
