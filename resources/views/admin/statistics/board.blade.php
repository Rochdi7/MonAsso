            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center mb-1">
                        <h3 class="mb-0">
                            {{ $totalActiveMembers }} <small class="text-muted">/ total active members</small>
                        </h3>
        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <h4>{{ number_format(array_sum($cashflowValues->toArray()), 0, '.', ' ') }} MAD
                    ...
                    <div class="row justify-content-center g-3 text-center mb-3">
                        ...
                        <div class="col-6">
                            <div class="overview-product-legends">
                                ...
                                <h4 class="mb-0">{{ floor(($activeEvents ?? 0) * 0.75) }}</h4>
                            </div>
                        </div>
                    </div>
...
@section('scripts')
    ...
    <script>
        const memberGrowthData = {
            6: {
                categories: {!! json_encode($memberGrowthChartData[6]['categories']) !!},
                series: [{
                    name: 'New Members',
                    data: {!! json_encode($memberGrowthChartData[6]['data']) !!}
                }]
            },
            12: {
                categories: {!! json_encode($memberGrowthChartData[12]['categories']) !!},
                series: [{
                    name: 'New Members',
                    data: {!! json_encode($memberGrowthChartData[12]['data']) !!}
                }]
            },
            all: {
                categories: {!! json_encode($memberGrowthChartData[12]['categories']) !!},
    ...
    <script>
        const monthlyReportChart = new ApexCharts(document.querySelector("#monthly-report-graph"), {
            ...
            series: [{
                name: 'Cotisations',
                data: {!! json_encode($cashflowValues) !!}
            }],
            xaxis: {
                categories: {!! json_encode($cashflowLabels) !!}
            },
            colors: ['#22c55e'],
            dataLabels: {
                enabled: false
