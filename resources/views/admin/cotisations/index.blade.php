                    ...
                    <table class="table table-hover align-middle" id="pc-dt-simple">
                        <thead>
                            <tr>
                                ...
                                @unless($isClient)<th>Association</th>@endunless
                                <th>Year</th>
                                <th>Amount</th>
                                <th>Status</th>
                                <th>Paid At</th>
                        ...
                        <tbody>
                            @forelse($cotisations as $cotisation)
                                <tr>
                                    ...
                                    @unless($isClient)<td>{{ $cotisation->association->name ?? '—' }}</td>@endunless
                                    <td>{{ $cotisation->year }}</td>
                                    <td>{{ number_format($cotisation->amount, 2) }} MAD</td>
                                    <td>
                                        @switch($cotisation->status)
                                            @case(1)
                                                <span class="badge bg-light-success text-success">✔ Paid</span>
                                                @break
                                            @case(2)
                                                <span class="badge bg-light-danger text-danger">✖ Overdue</span>
                                                @break
                                            @case(3)
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ $isClient ? 6 : 8 }}" class="text-center text-muted">No cotisations found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
