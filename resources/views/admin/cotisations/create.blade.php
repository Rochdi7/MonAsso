...
<div class="mb-3 col-md-6">
        ...
        @if ($authUser->hasRole('super_admin'))
            @foreach($associations as $association)
                <option value="{{ $association->id }}" 
                    @selected(old('association_id', isset($user) ? $user->association_id : '') == $association->id)>
                    {{ $association->name }}
                </option>
            @endforeach
        @elseif ($authUser->hasRole('admin'))
            @php
                $adminAssociation = $associations->firstWhere('id', $authUser->association_id);
            @endphp
    ...
    {{-- Hidden input to submit association_id when disabled --}}
    ...
    @enderror
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    ...
                                    <option value="2" @selected(old('status') == 2)>Overdue</option>
                                    <option value="3" @selected(old('status') == 3)>Rejected</option>
                                </select>
                                @error('status')
                                    <div class="text-danger mt-1">{{ $message }}</div>
                                @enderror
